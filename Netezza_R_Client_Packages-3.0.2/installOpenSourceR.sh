...
check_src () {
	...
	if [ $cnt -eq 0 ]; then
		if [ "$DL" = "1" ]; then # Try to download the source code
			...
			wget -t 2 -T 30 -P "$SRCDIR" "$url" 2>&1 | log_and_progress 5
...
create_spu_script() {
	...
	local libpath="-L${SPU64_DIR}/lib64 -L${SPU64_DIR}/usr/lib64 -L${SPU64_DIR}/generic/x86_64-generic-linux-gnu/lib64"
	local rpath="${SPU64_DIR}/lib64:${SPU64_DIR}/usr/lib64:${SPU64_DIR}/generic/x86_64-generic-linux-gnu/lib64"
	...
	cat << end-of-input | sed "s/^\t\+//" > ${SCRIPT}
		...
		export PATH="${SPU64_DIR}/bin:${SPU_DIR}/bin:${PERL_SPU_DIR}/bin:\${PATH}"
		export LD_LIBRARY_PATH=${SPU64_DIR}/lib64:${SPU64_DIR}/usr/lib64
		export LDFLAGS="-m64 ${linker_optimization} ${libpath} -Wl,-rpath=${rpath}"
		export CPPFLAGS="-m64 -I${SPU64_DIR}/include"
...
expected_r () {
	...
	[ "$inza" = "3.0.2" ] && expected_r_version="3.0"
...
install_r () {
	...
	READLINE_SRC_URL="ftp://ftp.cwru.edu/pub/bash/readline-6.2.tar.gz"
	...
	if [ "${DO_SPU}" = "1" ]; then
		...
		echo "export LDFLAGS=\"-L${DEV_SPU64_DIR}/lib64 \${LDFLAGS}\"" >> ${SCRIPT} 
	...
	if [ "${DO_HOST}" = "1" ]; then
		(
		export PATH="${HOST64_DIR}/bin:${HOST64_DIR}/x86_64-rhel5-linux-gnu/bin:${HOST_DIR}/bin:${PERL_HOST_DIR}/bin:/usr/bin:/bin"
		...
		libpath="-L${HOST64_DIR}/lib64 -L${HOST64_DIR}/usr/lib64 -L${HOST64_DIR}/x86_64-rhel5-linux-gnu/lib64"
		[ "${MKL_LIB_PATH}" != "" ] && libpath="-L${MKL_LIB_PATH} ${libpath}"
		rpath="${HOST64_DIR}/lib64:${HOST64_DIR}/usr/lib64:${HOST64_DIR}/x86_64-rhel5-linux-gnu/lib64"
		[ "${MKL_LIB_PATH}" != "" ] && rpath="${MKL_LIB_PATH}:${rpath}"
		
		export LDFLAGS=" -m64 ${LINK_OPTIMIZATION} ${libpath} -Wl,-rpath=${rpath}"
		export CPPFLAGS="-m64 -I${HOST64_DIR}/include"
