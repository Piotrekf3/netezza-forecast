...
nzMerge <- function(x, y, by = intersect(x@cols, y@cols), by.x = by, by.y = by, all = FALSE, all.x = all, all.y = all, sort = TRUE, suffixes = c("_x", "_y"), table = NULL, on = NULL, type = NULL) {
	
	#  error check x, y
	if (!is.element(class(x), "nz.data.frame")) {
		stop("nzMerge can only merge nz.data.frame")
	}
	if (!is.element(class(y), "nz.data.frame")) {
		stop("nzMerge can only merge nz.data.frame")
	}
	
	# if on not null, the nzJoin function is called
	if(!is.null(on)||!is.null(type)){
		stop("Parameters 'on' and 'type' are no longer supported.")
	}
	else{
	...
	if(!is.null(x@where)) {
		xView <- nzCreateView(x);
		x <- nz.data.frame(xView);
	} else 
		xView <- NULL;
	
	if(!is.null(y@where)) {
		yView <- nzCreateView(y);
		y <- nz.data.frame(yView);
	} else 
		yView <- NULL;
	
	
	query = paste("create table ", table, " as select ", outputColumnsForQuery, " from ", nzdf.qualified.name(x), " x", joinType, nzdf.qualified.name(y), " y", onClause, sep = "")
	nzQuery(query)

	output = nz.data.frame(table)
