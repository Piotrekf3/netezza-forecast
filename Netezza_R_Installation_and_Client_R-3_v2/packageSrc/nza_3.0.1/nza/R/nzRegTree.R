...
`nzRegTree` <- function(
	...
	if (format == "tree") {
		result <- nzRegTree.format.tree(model, form, data)
	} else if (format == "rpart") {
		result <- nzRegTree.format.rpart(model, form, data)
	} else if (format == "raw") {
		result <- nzRegTree.format.raw(model)
	}
...
nzRegTree.format.tree <- function(model, form, data) {
	...
	# compute tree$frame$dev
	dev <- NA
	
	# compute tree$frame$yval
	yval <- as.numeric(as.character(nodes$CLASS))
	...
	# create tree$frame
	frame <- data.frame(var = var, n = n, dev = dev, yval = yval, splits = I(splits), stringsAsFactors = F)		
	...
	# create tree
	tree <- list(frame = frame, terms = terms, call = sys.call(-1), model=model, modelTable=modelTable)
	
	# set attributes
	attr(tree, "xlevels") <- xlevels
	attr(tree, "format") <- "tree"
	
	# set class and return
	class(tree) <- c("nzRegTree")
	return(tree)
}

#------------------------------------------------------------------------------

nzRegTree.format.rpart <- function(model, form, data) {
	# read results from database
	modelStr <- paste("INZA.NZA_META_",model,"_NODES",sep="")
	nodes <- nzr::as.data.frame(nz.data.frame(modelStr))
	modelStr <- paste("INZA.NZA_META_",model,"_PREDICATES",sep="")
...
#	yprob1 <- matrix(0, nrow=nrow(nodes), ncol=1)
#	yprob2 <- matrix(0, nrow=nrow(nodes), ncol=1)
...
#	yval2 <- cbind(yprob3, yprob1, yprob2, yprob4)
	...
	dummy <- rpart:::rpart.anova(0,NULL,NULL,0)
	functions <- list()
...
nzRegTree.format.raw <- function(modelname) {
	...
	modelStr <- paste("INZA.NZA_META_",modelname,"_NODES",sep="")
	nodes <- nzr::as.data.frame(nz.data.frame(modelStr))
	modelStr <- paste("INZA.NZA_META_",modelname,"_PREDICATES",sep="")
	predicates <- nzr::as.data.frame(nz.data.frame(modelStr))
...
# wrapper for predict
`predict.nzRegTree` <- function(object, newdata, id="id", ...) {
	...
	tryCatch({
				callSP("nza..PREDICT_REGTREE ", model=object$model, intable=tmpView, id=id, outtable=outtable, ...)
			}, error = function(e, tmpView) {
				# in case of error, drop view and let user know, what happend
				stop(e)
			}, finally = {
